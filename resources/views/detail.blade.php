  ...
  <div class="container px-4 px-lg-5">
    <div class="row">
      ...
      <div class="col-6 col-md-6">
        ...
        <p>{{ $detail->softwere }}</p>
        <p class="col-sm-4 text-white text-center p-3 
        @if($detail->status=='menunggu') {{ 'bg-warning' }} 
        @elseif($detail->status=='diterima') {{ 'bg-success' }}
        @else {{ 'bg-danger' }}  
        @endif">{{ $detail->status }}</p>
        <p>{{ $detail->created_at }}</p>
        <p>{{ $detail->tanggal_running }}</p>
        <p>{{ $detail->tanggal_selesai }}</p>
        <p>{{ $detail->bukti_running==''?'-':$detail->bukti_running }}</p>
        <p>{{ $detail->bukti_selesai==''?'-':$detail->bukti_selesai }}</p>
        @endforeach
      </div>
    </div>
  </div>
